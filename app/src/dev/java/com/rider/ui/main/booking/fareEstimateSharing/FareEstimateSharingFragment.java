package com.rider.ui.main.booking.fareEstimateSharing;
...
import com.rider.model.request_model.BookCabRequestModel;
import com.rider.model.request_model.CheckFareRequestModel;
import com.rider.model.web_response.BaseWebServiceModelResponse;
import com.rider.model.web_response.BookCabResponseModel;
import com.rider.ui.main.dialog.confirmBooking.ConfirmBookingDialog;
import com.rider.ui.main.dialog.paymentmode.PaymentModeDialog;
import com.rider.ui.main.dialog.promocode.PromocodeDialog;
import com.rider.ui.main.dialog.reminingPayment.RemainingPaymentDialog;
import com.rider.ui.main.dialog.seatDialog.SeatAvailableDialog;
import com.rider.ui.main.dialog.walletrecharge.WalletRechargeDialog;
import com.rider.ui.main.sidemenu.payment.wallet.WalletFragment;
import com.rider.ui.utilities.MapHandler;
...
public class FareEstimateSharingFragment extends AppBaseFragment implements DistanceMatrixHandler.DistanceMatrixListener {
    ...
    private TextView tv_seat_available;
    ...
    public int getLayoutResourceId() {
        return R.layout.fragment_fare_estimate_sharing;
    }
    ...
    public void initializeComponent() {
        ...
        ll_select_seat = getView().findViewById(R.id.ll_select_seat);
        tv_seat_available = getView().findViewById(R.id.tv_seat_available);
        ll_select_seat.setOnClickListener(this);
    ...
    private void updateDriverAvailableTime() {
        if (getMapHandler().cabTypeFragment != null &&
          ...
          /*  VehicleTypeModel vehicleTypeModel = getMapHandler().cabTypeFragment.
                    getVehicleTypeData(getMapHandler().fareEstimateModel.getPackages().get(0).getFares().get(0).getVehicle());
*/
//            VehicleTypeModel vehicleTypeModel = getMapHandler().fareEstimateModel.getPackages().get(0).getFares().get(0).getVehicle();
    ...
    private void updateScheduledUi() {
        BookCabModel bookCabModel = getMapHandler().fareEstimateModel;
        List<RentalPackageModel> packages = bookCabModel.getPackages();
        if (packages != null && packages.size() > 0) {
            RentalPackageModel rentalPackageModel = packages.get(0);
            List<RentalFareModel> fares = rentalPackageModel.getFares();
            if (fares != null && fares.size() > 0) {
                RentalFareModel rentalFareModel = fares.get(0);
                if (isValidObject(rentalFareModel)) {
    ...
    private void showSelectSeatDialog() {
        ...
        if (packages != null && packages.size() > 0) {
            ...
            if (fares != null && fares.size() > 0) {
                RentalFareModel rentalFareModel = fares.get(0);
                if (isValidObject(rentalFareModel)) {
                    SeatAvailableDialog seatAvailableDialog = new SeatAvailableDialog();
                    seatAvailableDialog.setFareModel(rentalFareModel);
                    seatAvailableDialog.setSeatAvailableListener(new SeatAvailableDialog.SeatAvailableListener() {
                        @Override
                        public void onSeatSelected(SeatAvailableDialog seatAvailableDialog, String seat, String seatCharge) {
                            tv_seat_available.setText(seat);
                            tv_fare_estimate.setText(currency_symbol + seatCharge);
                            seatAvailableDialog.dismiss();
                        }
                    });
                    seatAvailableDialog.show(getFragmentManager(), seatAvailableDialog.getClass().getSimpleName());
                }
            }
        }
    ...
    private void confirmationDialog() {
        confirmationDialog = ConfirmBookingDialog.getNewInstance("");
        ...
        confirmationDialog.setConfirmationDialogListener(new ConfirmBookingDialog.ConfirmationDialogListener() {
            ...
            @Override
            public String getAvailableTime() {
                return FareEstimateSharingFragment.this.getAvailableTime();
            }
            ...
            public FareModel getFare() {
                if (getMapHandler().fareEstimateModel != null) {
                    BookCabModel bookCabModel = getMapHandler().fareEstimateModel;
                    RentalFareModel rentalFareModel = bookCabModel.getPackages().get(0).getFares().get(0);
                    String seat = tv_seat_available.getText().toString().trim();
                    if (seat.equalsIgnoreCase("2 seat")) {
                        rentalFareModel.setSaetCount(2);
                    } else {
                        rentalFareModel.setSaetCount(1);
                    }
    ...
    private void bookCab(ConfirmBookingDialog dialogFragment) {
        ...
        if (bookCabModel != null) {
            ...
            try {
                ...
                model.booking_running_type = BOOKING_RUNNING_TYPE_SHARING;
            ...
            model.seat_count = bookCabModel.getPackages().get(0).getFares().get(0).getSaetCount();
            model.cab_type_id = bookCabModel.getPackages().get(0).getFares().get(0).getVehicle().getId();
            ...
            model.taxes = bookCabModel.getPackages().get(0).getFares().get(0).getTotal_tax().getTaxes();
            model.fare = bookCabModel.getPackages().get(0).getFares().get(0);
